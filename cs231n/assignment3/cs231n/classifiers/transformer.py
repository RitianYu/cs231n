...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        cap_embed_pos=self.positional_encoding(cap_embed)
        features_proj= self.visual_projection(features)
        tgt_mask=torch.flip(torch.tril(torch.ones(T,T)),dims=[1])
        memory=features_proj.unsqueeze(1)
        output=self.transformer(cap_embed_pos,memory,tgt_mask)
