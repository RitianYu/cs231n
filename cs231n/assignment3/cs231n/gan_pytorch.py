...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss_data=bce_loss(logits_real,true_labels.squeeze())
    loss_gen=bce_loss(logits_fake,fake_labels.squeeze())
    loss=loss_data+loss_gen
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss_data=0.5*torch.mean(torch.square(scores_real-1),dim=0)
    loss_gen=0.5*torch.mean(torch.square(scores_fake),dim=0)
    loss=loss_data+loss_gen
...
def ls_generator_loss(scores_fake):
    ...
    loss=0.5*torch.mean(torch.square(scores_fake-1),dim=0)
...
def run_a_gan(D, G, D_solver, G_solver, discriminator_loss, generator_loss, loader_train, show_every=250,
    ...
    for epoch in range(num_epochs):
        for x, _ in loader_train:
            ...
            logits_real = D(2* (real_data - 0.5)).type(dtype)  #这里算D(x)的时候为什么要用 2*(real_data-0.5)?

            g_fake_seed = sample_noise(batch_size, noise_size).type(dtype) 
            fake_images = G(g_fake_seed).detach()  #先取出一个分布z，然后算出G(z)
            logits_fake = D(fake_images.view(batch_size, 1, 28, 28))  #将G(z) reshape成image的size，然后算出D(G(z))
