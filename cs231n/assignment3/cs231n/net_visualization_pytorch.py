...
def compute_saliency_maps(X, y, model):
    ...
    scores=model(X)  #计算模型的输出结果,这是一个N*M的矩阵
    scores=scores.gather(1, y.view(-1, 1)).squeeze() #scores中每一行只有一列是正确类别的得分，所以要在每一行都选出正确类别的分数
    gradTensor=torch.ones_like(scores.detach())
    scores.backward(gradTensor)
...
def make_fooling_image(X, target_y, model):
    ...
    for i in range(100):
        scores_fooling = model(X_fooling)
        _, index_y = torch.max(scores_fooling,dim=1)
        if index_y == target_y:
            print("success")
            print("end in %d iterations"%(i))
            break
        scores_target= scores_fooling[0,target_y]
