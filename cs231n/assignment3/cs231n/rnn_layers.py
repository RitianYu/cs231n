...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    out_h=np.dot(prev_h,Wh)
    out_x=np.dot(x,Wx)
    next_h=np.tanh(out_h+out_x+b)
...
def rnn_step_backward(dnext_h, cache):
    ...
    # 激活函数tanh的导数为 1-tanh^2， 设tanh=s,则 dtanh=1-s^2
    next_h, prev_h, x, Wx, Wh, b = cache
    dout_h=dnext_h * (1-np.square(next_h))
    dout_x=dnext_h * (1-np.square(next_h))
    dprev_h=np.dot(dout_h,Wh.T)
    dWh=np.dot(prev_h.T,dout_h)
    dx=np.dot(dout_x,Wx.T)
    dWx=np.dot(x.T,dout_x)
...
def rnn_backward(dh, cache):
    ...
    for i in range(T):
        if i<T-1:
            cache=(h[:,T-i-1,:],h[:,T-i-2,:],x[:,T-i-1,:],Wx,Wh,b)
            dx[:,T-i-1,:], dprev_h, dWx[T-i-1], dWh[T-i-1], db[T-i-1] = rnn_step_backward(dnext_h, cache)
            dnext_h=dprev_h+dh[:,T-i-2,:]
        else:
            cache=(h[:,0,:],h0,x[:,0,:],Wx,Wh,b)
            dx[:,0,:], dh0, dWx[0], dWh[0], db[0] = rnn_step_backward(dnext_h, cache)
