    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        for i in range(max_len):
            for j in range(embed_dim):
                if j%2==0:
                    pe[0,i,j]=math.sin(i*math.pow(10000,-j/embed_dim))
                else:
                    pe[0,i,j]=math.cos(i*math.pow(10000,-(j-1)/embed_dim))
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        H = self.n_head 
        Q=self.query(query).reshape(N,S,H,E//H).transpose(1,2)   # Q: N,H,S,K   K=E/H
        K=self.key(key).reshape(N,T,H,E//H).transpose(1,2)     # K: N,H,T,K   K=E/H
        V=self.value(value).reshape(N,T,H,E//H).transpose(1,2)   # V: N,H,T,K   K=E/H
        K_T=K.transpose(2,3)
        Es=torch.matmul(Q,K_T)/math.sqrt(E/H)       # Es: N,H,S,T   A: N,H,S,T
        if attn_mask != None:
            Es=Es.masked_fill(attn_mask==False,float("-inf"))
        A=F.softmax(Es,dim=3)    # 在维度T上做softmax，因为Value向量是T维的，需要对其进行加权
        Y=torch.matmul(self.attn_drop(A),V)    # Y: N,H,S,E/H
